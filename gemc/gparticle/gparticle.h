 ...
 *
 * These values are returned by \ref Gparticle::shootParticle and reflect the
 * actual randomized particle passed to Geant4.
 */
...
public:
	 ...
	 * \return Runtime values for every generated primary.
	 */
	[[nodiscard]] std::vector<GparticleRuntimeRecord> shootParticle(G4ParticleGun* particleGun,
...
private:
	...
	[[nodiscard]] double calculateMomentum() const;
	 ...
	 */
	[[nodiscard]] double calculateKinEnergy(double mass) const;

	/** \brief Computes the beam direction unit vector from randomized theta and phi values. */
	[[nodiscard]] G4ThreeVector calculateBeamDirection(double thetaRad, double phiRad) const;

	/** \brief Computes the randomized vertex position according to the configured model. */
	[[nodiscard]] G4ThreeVector calculateVertex() const;
