...
vector<G4SceneText> getSceneTexts(const std::shared_ptr<GOptions>& gopts) {
	...
	for (auto g4t_item : g4t_node) {
		...
		st_item.text  = gopts->get_required_variable_in_option<string>(g4t_item, "text");
		...
		st_item.z     = gopts->get_optional_variable_in_option<double>(g4t_item, "z");
...
GOptions addSceneTextsOptions() {
	...
	vector<GVariable> g4text = {
		{"text", goptions::REQUIRED, "string with the text to be displayed"},
		...
		{"z", std::nullopt, "z position of 3D text (required when kind is 3D)"},
