...
inline std::filesystem::path gemc_root() {
	...
	// Case 1: executable is directly in the build directory (legacy build-tree layout)
	if (exe_dir.filename() == "build") {
		root = exe_dir.parent_path();
	}
	// Case 2: executable is in a bin/ subdirectory — installed or build/bin/
	else if (exe_dir.filename() == "bin") {
		root = exe_dir.parent_path();
		if (std::filesystem::exists(root / "build.ninja")) {
			// Running from build/bin/: step up one more level to match the legacy build-tree root
			root = root.parent_path();
		} else if (!std::filesystem::exists(root / "lib")) {
