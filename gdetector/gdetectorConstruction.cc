...
G4ThreadLocal G4bool GDetectorConstruction::fConstructedSDandField = false;
...
//					sdManager->AddNewDetector(sensitiveDetectorsMap[digitizationName]);
				...
				GSensitiveDetector *thisSensitive = new GSensitiveDetector("ch", gopt, gDynamicDigitizationMapGlobalInstance);
				sdManager->AddNewDetector(thisSensitive);
				//SetSensitiveDetector(g4world->getG4Volume(volumeName)->getLogical(), thisSensitive);

				thisSensitive->registerGVolumeTouchable(volumeName, new GTouchable(digitizationName, gvolume->getGIdentity()));
			//	sensitiveDetectorsMap[digitizationName]->registerGVolumeTouchable(volumeName, new GTouchable(digitizationName, gvolume->getGIdentity()));
				...
				cout << " ASD " << g4world->getG4Volume(volumeName)->getLogical() << " " << thisSensitive << endl;

				// this
//				g4world->getG4Volume(volumeName)->getLogical()->SetSensitiveDetector(thisSensitive);

				// SetSensitiveDetector is called by G4VUserDetectorConstruction call
			//	SetSensitiveDetector(g4world->getG4Volume(volumeName)->getLogical(), sensitiveDetectorsMap[digitizationName]);
