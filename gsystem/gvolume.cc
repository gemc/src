...
std::ostream& operator<<(std::ostream& stream, const GVolume& gVol)
{
    ...
    if (gVol.copyOf != "" && gVol.copyOf != UNINITIALIZEDSTRINGQUANTITY) stream << "   - copyOf:          " << gVol.
        copyOf << std::endl;
    if (gVol.solidsOpr != "" && gVol.solidsOpr != UNINITIALIZEDSTRINGQUANTITY) stream << "   - solidsOpr:            "
        << gVol.solidsOpr << std::endl;
    if (gVol.type != "" && gVol.type != UNINITIALIZEDSTRINGQUANTITY) stream << "   - Type:            " << gVol.type <<
        std::endl;
    if (gVol.parameters != "" && gVol.parameters != UNINITIALIZEDSTRINGQUANTITY) stream << "   - Parameters:      " <<
    ...
    if (gVol.emfield != "" && gVol.emfield != UNINITIALIZEDSTRINGQUANTITY) stream << "   - E.M. emfield:      " << gVol.
        emfield << std::endl;
    if (gVol.digitization != "" && gVol.digitization != UNINITIALIZEDSTRINGQUANTITY) stream << "   - Digitization:    "
        << gVol.digitization << std::endl;
    if (gVol.gidentity != "" && gVol.gidentity != UNINITIALIZEDSTRINGQUANTITY) stream << "   - GIdentity:       " <<
