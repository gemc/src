...
void GSystem::addGVolume(vector <string> pars, int verbosity) {
    ...
    if (gvolumesMap->find(volume_name) == gvolumesMap->end()) {
        ...
        if (verbosity >= GVERBOSITY_DETAILS) {
            cout << GSYSTEMLOGHEADER << "Adding gVolume <" << volume_name << "> to gvolumesMap with name as key" << endl;
        }
        (*gvolumesMap)[volume_name] = new GVolume(name, pars);

        if (verbosity >= GVERBOSITY_DETAILS) {
            cout << *(*gvolumesMap)[volume_name];
        }

    } else {
        cerr << FATALERRORL << "a gVolume with the name <" << volume_name << "> already exists. " << endl;
