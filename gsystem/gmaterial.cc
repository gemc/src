...
void GMaterial::getMaterialPropertyFromString(string parameter, string propertyName) {
    ...
    if (removeLeadingAndTrailingSpacesFromString(parameter) == UNINITIALIZEDSTRINGQUANTITY) {
        return;
    }
    ...
    while (!parameterComponents.eof()) {
        ...
        if (propertyName == "rayleigh") {
            ...
            if (indexOfRefraction.size() > 0 && indexOfRefraction.size() != photonEnergyVectorSize) {
                cerr << FATALERRORL << "indexOfRefraction size " << indexOfRefraction.size() << " mismatch: photonEnergy has size " << photonEnergyVectorSize
                     << endl;
                gexit(EC__GMATERIALOPTICALPROPERTYMISMATCH);
            }
            if (absorptionLength.size() > 0 && absorptionLength.size() != photonEnergyVectorSize) {
                cerr << FATALERRORL << "absorptionLength size " << absorptionLength.size() << " mismatch: photonEnergy has size " << photonEnergyVectorSize
                     << endl;
                gexit(EC__GMATERIALOPTICALPROPERTYMISMATCH);
            }
