...
ifstream *GSystemTextFactory::gSystemTextFileStream(GSystem *system, string SYSTEMTYPE, int verbosity) {
   ...
   // if (verbosity == GVERBOSITY_DETAILS) {
        cout << GSYSTEMLOGHEADER << "gSystemTextFileStream: filename: " << KGRN << fname << RST << endl;
    //}
    ...
    if (IN->good()) {
        if (verbosity == GVERBOSITY_SUMMARY) {
            cout << GSYSTEMLOGHEADER << "Opening file " << KGRN << fname << RST << endl;
        }
        return IN;
    } else {
        ...
        for (auto trialLocation: possibleLocationOfFiles) {
            ...
            if (IN->good()) {
                if (verbosity >= GVERBOSITY_SUMMARY) {
                    cout << GSYSTEMLOGHEADER << "Opening file " << KGRN << newName << RST << endl;
                }
