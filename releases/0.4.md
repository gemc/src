...
- Optical surfaces now work for CAD systems too: the CAD system factory reads the `mirrors` table from
  the geometry database, so a mesh volume that names a `GVolume.mirror` surface gets the same
  `G4OpticalSurface` treatment as native volumes.
- CAD systems now support `CopyOf` volumes: a CAD volume with a `copyOf` reuses the source mesh's solid
  and logical volume instead of loading a mesh of its own, so a single STL can be placed many times (e.g.
  the LTCC frame plates copied into all six sectors). The CAD `buildSolid` resolves the copy the same way
  the native factory does.
