...
- [Issue #62](https://github.com/gemc/src/issues/62): fix reload geometry : run/beamOn does not work after a reload
- [Issue #74](https://github.com/gemc/src/issues/74): add generator in the GUI with tabs fore each particle. 
- [Issue #84](https://github.com/gemc/src/issues/84): add photon_detector
- [Issue #86](https://github.com/gemc/src/issues/86): add options for background colors and num cloud points
- [Issue #89](https://github.com/gemc/src/issues/89): Pyvista installed on the container
- [Issue #93](https://github.com/gemc/src/issues/93): add pygemc repo for api and analyzer
- [Issue #94](https://github.com/gemc/src/issues/94): add barrel scintillator array example
- [Issue #95](https://github.com/gemc/src/issues/95): add text and arrow options to g4display
- [Issue #96](https://github.com/gemc/src/issues/96): fix detector not showing in the gui if running with just gemc -gui
- [Issue #97](https://github.com/gemc/src/issues/97): geometry reload crash
- [Issue #103](https://github.com/gemc/src/issues/103): [High] EventDispenser: run weights are not normalized before stochastic sampling
- [Issue #104](https://github.com/gemc/src/issues/104): [High] Run header stores the Geant4 internal run ID instead of the GEMC run number
- [Issue #105](https://github.com/gemc/src/issues/105): [High] Run-level normalization applied once per streamer causes double-normalization
- [Issue #106](https://github.com/gemc/src/issues/106): [High] Multiple same-format gstreamer outputs collapse into a single streamer
- [Issue #107](https://github.com/gemc/src/issues/107): [High] Multipole field parameters bypass unit parsing (stod/stoi ignore unit expressions)
- [Issue #108](https://github.com/gemc/src/issues/108): [High] Inverted dependency-stall check aborts geometry/material build when progress is made
- [Issue #109](https://github.com/gemc/src/issues/109): [High] Sphere vertex sampling compares squared radius against un-squared bound
- [Issue #110](https://github.com/gemc/src/issues/110): [High] Fix CAD loader mutating dirLocation in-place inside the per-file loop
- [Issue #111](https://github.com/gemc/src/issues/111): [High] Add missing opacity field in addVolumeFromFile so GVolume gets all 21 parameters
- [Issue #112](https://github.com/gemc/src/issues/112): [High] Run optical-property size validation once after parsing, not on every rayleigh token
- [Issue #113](https://github.com/gemc/src/issues/113): [High] ProcessHits mutates the persistent map GTouchable, leaking state across events
- [Issue #115](https://github.com/gemc/src/issues/115): [High] JSON streamer emits invalid JSON for multi-detector digitized output
- [Issue #116](https://github.com/gemc/src/issues/116): [High] JSON streamer never closes the event "header" object, nesting siblings inside it
- [Issue #117](https://github.com/gemc/src/issues/117): [High] add_rotation() emits a multi-token rotation string the C++ parser silently drops
- [Issue #118](https://github.com/gemc/src/issues/118): [High] set_rotation() then add_rotation() raises AttributeError due to inconsistent rotations type
- [Issue #119](https://github.com/gemc/src/issues/119): [High] make_polycone() length check uses `and` instead of `or`, missing single-array mismatches
- [Issue #120](https://github.com/gemc/src/issues/120): [High] make_trap() passes an extra `self` to already-bound methods, raising TypeError
- [Issue #121](https://github.com/gemc/src/issues/121): [Medium] `gemc help` with no topic crashes with std::logic_error (null argv read)
- [Issue #122](https://github.com/gemc/src/issues/122): [Medium] GConfiguration ignores constructor args; argparse defaults clobber factory/variation/runno
- [Issue #123](https://github.com/gemc/src/issues/123): [Medium] Tarball is not self-contained: docs claim static Geant4 linking but gemc is dynamically linked
- [Issue #124](https://github.com/gemc/src/issues/124): [Medium] EventDispenser::variation is never initialized, always passing empty string to digitization
- [Issue #125](https://github.com/gemc/src/issues/125): [Medium] YAML file detection uses substring match and misses YAML::BadFile, causing misparse and crashes
- [Issue #126](https://github.com/gemc/src/issues/126): [Medium] Worker run-data stashed every run but drained only when run streamers exist
- [Issue #127](https://github.com/gemc/src/issues/127): [Medium] GHit::nsteps() returns Es.size(), which is empty unless HitBit 0 is set
- [Issue #128](https://github.com/gemc/src/issues/128): [Medium] CSV run-mode rows print uninitialized event_number and thread_id
- [Issue #129](https://github.com/gemc/src/issues/129): [Medium] CSV row newline only written inside the double-observables loop, dropped when a hit has no doubles
- [Issue #130](https://github.com/gemc/src/issues/130): [Medium] GPhysics dereferences possibly-null physics list before the null check
- [Issue #131](https://github.com/gemc/src/issues/131): [Medium] Switches cannot be turned off from the CLI, breaking documented YAML-override precedence
- [Issue #132](https://github.com/gemc/src/issues/132): [Medium] Stop leaking heap ifstream on every ASCII system/material load
- [Issue #133](https://github.com/gemc/src/issues/133): [Medium] parseFileAndRemoveComments throws std::out_of_range on a comment before the first newline
- [Issue #134](https://github.com/gemc/src/issues/134): [Medium] gemc-sqlite CLI builds queries via raw f-strings (breakage + SQL injection)
- [Issue #135](https://github.com/gemc/src/issues/135): [Low] Make G4World own its G4Volume wrappers to stop a leak per volume on geometry reload
- [Issue #136](https://github.com/gemc/src/issues/136): [Low] Add rule-of-five to GFrameDataCollection to prevent double-free if copied
- [Issue #137](https://github.com/gemc/src/issues/137): [Low] Guard against null GetVisAttributes() in G4Ttree_item constructor
- [Issue #139](https://github.com/gemc/src/issues/139): [Low] convert_angle(out='gon') returns radians instead of gradians
- [Issue #169](https://github.com/gemc/src/issues/169): Checkout of untrusted code in a privileged context
