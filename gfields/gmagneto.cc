...
GMagneto::GMagneto(GOptions *gopts) {
    ...
    for (auto &jmultipole: jmultipoles) {
        ...
        cout << " jmultipole name: " << name << endl;
    }

    // instantiating gFieldManager and fieldFactory
//    GManager gFieldManager("GMagneto", gopts->getInt("gfieldv"));
//
//    map<string, GFieldFactory *> fieldFactory;
//
//    // registering factories in gFieldManager
//    // and adding them to fieldFactory
...
//    gFieldManager.RegisterObjectFactory<GFieldTextFactory>(GFIELDTEXTFACTORYLABEL);
//
//    if (fieldFactory.find(GFIELDTEXTFACTORYLABEL) == fieldFactory.end()) {
//        fieldFactory[GFIELDTEXTFACTORYLABEL] = gFieldManager.CreateObject<GFieldFactory>(GFIELDTEXTFACTORYLABEL);
//    }
//
//    for (auto &[gfieldName, gfield]: *gFieldMap) {
//
//        string factoryName = gfield->getFactoryName();
//
//        if (factoryName == GFIELDMAGNETICFACTORYLABEL) {
//            if (fieldFactory.find(factoryName) == fieldFactory.end()) {
//                gFieldManager.RegisterObjectFactory<GFieldMagneticFactory>(factoryName);
//                fieldFactory[factoryName] = gFieldManager.CreateObject<GFieldFactory>(factoryName);
//            }
//        } else if (factoryName == GFIELDGRADIENTFACTORYLABEL) {
//            if (fieldFactory.find(factoryName) == fieldFactory.end()) {
//                gFieldManager.RegisterObjectFactory<GFieldGradientFactory>(factoryName);
//                fieldFactory[factoryName] = gFieldManager.CreateObject<GFieldFactory>(factoryName);
//            }
//        }
//    }
