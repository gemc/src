...
G4Commands::G4Commands(QWidget* parent) : QWidget(parent) {
	// Summary: Build the overall UI layout (search, commands tree + help, history, command prompt).
	...
	// Search entry: used to filter the tree model in real time.
	...
	// Commands tree and help are placed side-by-side.
	...
	// Left: the commands tree (built from Geant4 UI command tree).
	create_geant4_commands_widget();
	commands_help_layout->addWidget(w_commands);

	// Right: the help on individual commands or command directories.
	w_help = new QTextEdit(commands_help_splitter);
	w_help->setReadOnly(true);

	// History area: executed commands (deduplicated).
	...
	// Command entry: pressing Enter triggers execution.
	...
	// Allocate more horizontal space to the help pane by default.
	commands_help_splitter->setSizes(QList<int>() << 300 << 800);

	// Putting all together into a vertical layout.
...
void G4Commands::create_geant4_commands_widget() {
	// Summary: Build the QTreeView model by walking the Geant4 UI command tree.

	// Read current search text (used by filterTreeItems()).
	QString search_text = w_search->text();
	(void)search_text; // Not used immediately here; kept as a convenient debug hook.
	...
	// Add top-level command directories to the model.
	G4int g4_commands_tree_size = g4_commands_tree->GetTreeEntry();
	for (int a = 0; a < g4_commands_tree_size; a++) {
		// Each top-level entry is a command directory node; children are populated recursively.
		QStandardItem* newItem = new QStandardItem(
			QString((char*)g4_commands_tree->GetTree(a + 1)->GetPathName().data()).trimmed());
		model->appendRow(newItem);

		// Add child directories and commands.
		create_child_help_tree(newItem, g4_commands_tree->GetTree(a + 1));
	}

	// Read-only: prevent accidental edits to the command path strings.
	w_commands->setEditTriggers(QAbstractItemView::NoEditTriggers);

	// Selection drives help display; double-click pastes selection into the command prompt.
...
void G4Commands::filterTreeItems() {
	// Summary: Hide/show top-level rows depending on whether they (or their descendants) match the search string.
...
bool G4Commands::filterItem(QStandardItem* item, const QString& search_text) {
	...
	// Apply visibility at this node's location in the tree.
...
void G4Commands::create_child_help_tree(QStandardItem* parent, G4UIcommandTree* aCommandTree) {
	// Summary: Populate children for a given Geant4 tree node (sub-directories first, then leaf commands).

	if (parent == nullptr || aCommandTree == nullptr) return;

	// Add child directories (sub-trees).
	...
	// Add commands at this node (leaf entries).
...
void G4Commands::execute_command() {
	// Summary: Execute the typed command through Geant4 UI manager and update history.
	...
	// Avoid duplicate history entries (simple linear scan, small list expected).
	bool exists = false;
	...
	// Clear command field after execution to signal completion.
	w_command->clear();
}

void G4Commands::recall_history_item_on_double_click(QListWidgetItem* item) {
	// Summary: Load the selected history command into the input field.
...
void G4Commands::display_help_from_selection() {
	// Summary: Determine whether selection is a command or a directory and show help/title accordingly.
	...
	if (command) {
		// Leaf command: show detailed guidance and parameter info.
		w_help->setText(get_command_g4help(command));
	}
	else {
		// Directory node: show the title (if found).
...
void G4Commands::paste_help_selection_item() {
	// Summary: Refresh help and paste the selected command/directory text into the prompt.
...
QString G4Commands::get_command_g4help(const G4UIcommand* aCommand) {
	// Summary: Format Geant4 UI command help into a human-readable text block.
	...
	// If there is no meaningful content, return an empty string (keeps UI tidy).
	if ((commandPath == "") && (rangeString == "") && (n_guidanceEntry == 0) && (n_parameterEntry == 0)) { return txt; }

	// CommandPath ending with '/' denotes a directory in Geant4 UI; otherwise it's a concrete command.
	...
	if (n_parameterEntry > 0) {
		...
		// Re-implementation from G4UIparameter.cc (mirrors Geant4 formatting for consistency).
