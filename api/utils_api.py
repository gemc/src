...
class GConfiguration():
	def __init__(self, system, factory='TEXT', parser=None):
		self.system = system
		self.runno = 1
	...
	def printC(self):
		print()
		print("    ❖ Sci-g configuration for system <" + gcolors.BOLD + str(self.system) + gcolors.END + "> : " + str(
			self.description))
...
def defined_parser(desc: str):
	"""Creates and returns an argument parser with common options."""
	parser = argparse.ArgumentParser(description=desc)

	# Add option to select the database type
	# it can be either a directory path or a sqlite file
