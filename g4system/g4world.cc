...
G4World::G4World(GWorld *gworld, GOptions *gopts) {
    ...
    do {
        ...
        for (auto &system: *gworld->getSystemsMap()) {
            ...
            for (auto &[volumeName, gvolume]: *system.second->getGVolumesMap()) {
                if (g4systemFactory.find(g4Factory) != g4systemFactory.end()) {
                    ...
                    if (verbosity == GVERBOSITY_DETAILS) {
                        G4cout << G4SYSTEMLOGHEADER << "using factory <" << KYEL << g4Factory << RST << ">, to build g4volume <" << KYEL << gvolume->getG4Name()
                               << RST << ">" << endl;
                    }
            ...
            if (verbosity == GVERBOSITY_DETAILS) {
                G4cout << G4SYSTEMLOGHEADER << system.first << " : " << thisIterationRemainingVolumes.size() << " remaining motherless g4volumes to be built: "
                       << endl;
        ...
        if (isChemical) {
            if (NISTman->FindOrBuildElement(componentName) == nullptr) {
                if (verbosity == GVERBOSITY_SUMMARY) {
                    G4cout << G4SYSTEMLOGHEADER << " element component " << componentName << " needed by material " << materialName << " not found yet "
                           << endl;
                }
                return false;
            } else {
            ...
            if (NISTman->FindOrBuildMaterial(componentName) == nullptr) {
                if (verbosity == GVERBOSITY_SUMMARY) {
                    G4cout << G4SYSTEMLOGHEADER << "material component " << componentName << " needed by material " << materialName << " not found yet "
                           << endl;
                }
                return false;
            } else {
